...
This chapter covers two routes: **uploading** a CV you already have, and **building** one from scratch with a template. Both end at the same place — at least one CV file saved under My CVs and ready to attach to applications.
...
3. Wait for the AI parser to finish. It reads your CV and extracts:
...
- Skills inferred too aggressively (the parser may add things you only mentioned in passing)
- Names of past employers misspelled or truncated
...
## Step 3 — Create a second CV (optional but recommended)

If you're applying to more than one type of role, create a second CV tailored to the second type. For example:
...
- It becomes selectable when you apply to jobs (Chapter 11)
- It gets used by **Auto Match** (Chapter 9) to score job recommendations
- It powers **CV Review** (Chapter 6), the next chapter
- AI matching reads it to generate match scores and missing-skill lists on each application
...
- [ ] Optionally: created a second CV for a different type of role
