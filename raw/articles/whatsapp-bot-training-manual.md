# Sebenza Hub WhatsApp Bot — Training Manual
...
The Sebenza Hub WhatsApp Bot is an AI-powered recruitment assistant that connects with candidates through WhatsApp. It handles the full recruitment lifecycle: job browsing, applications, CV parsing, candidate screening, assessments, career coaching, and human escalation.
...
| Role | What they do |
|------|-------------|
| **Candidates** | Chat with the bot on WhatsApp to find jobs, apply, upload CVs, and get career advice |
| **Recruiters** | Monitor conversations, manage screening, handle escalations, run campaigns |
| **Admins** | Configure the bot, train intents, build flows, monitor analytics, ensure compliance |
...
3. Use the **Playground** to test: type a message and see what the bot would respond
4. Check the **Unmatched** section regularly — these are messages the bot couldn't handle
...
Bot: Welcome to Sebenza Hub — your AI-powered recruitment assistant!
...
```
Bot: Hello! Welcome to Sebenza Hub!
...
```
Bot: How can I help you today? Choose an option below:
...
Bot: Found 5 jobs matching "developer":
...
```
Bot: Great! Let's get your application started.
...
Bot: Perfect. Now send me your CV as a PDF or image.
```

If the candidate has an existing profile:

```
Bot: Would you like to apply with your existing profile
     or upload a new CV?
...
```
Bot: What email address did you use to apply?

Candidate: john@example.com

Bot: Hi John! Here are your recent applications:
...
```
Bot: I'm your career coach! Ask me anything about job searching,
...
Bot: Here are my top tips for technical interviews in SA:
...
When escalated, the bot stops auto-responding and a human recruiter takes over in the Team Inbox.
...
- **Playground** — Type messages and see real-time bot responses
- **Unmatched Log** — Messages the bot couldn't handle (use these to create new intents)
...
- See what the bot detected at each message: intent, confidence, state transition, sentiment
...
5. The bot sends questions via WhatsApp automatically
...
The bot analyzes every text message for sentiment:
...
**Auto-escalation** triggers when 3+ consecutive messages are negative (score < -50). The bot stops responding and alerts a recruiter.
...
The candidate selects their language on first contact. The bot remembers the preference for all future interactions.

### 8.3 POPIA Compliance

The bot enforces South African data protection law:
...
Recruiters can take over any conversation from the bot:

1. Go to Conversations tab
2. Find the conversation
3. Click **Takeover**
4. Send messages manually
5. When done, click **Release** to return control to the bot

While taken over, the bot does not auto-respond.
...
Yes — the bot collects name and email. CV upload is optional but recommended for better matching.
...
No. Once a candidate types STOP, they are opted out and the bot will not send messages. They must re-engage by sending a new message themselves.

**How many languages does the bot support?**
...
Go to Bot Training > FAQs, click "Add FAQ", enter the question and answer. The bot will start using it immediately.

**What happens when the bot can't understand a message?**
The bot sends the fallback message (configurable in Settings). The unmatched message is logged for review in the Bot Training > Unmatched section.

---

*Last updated: March 2026*
*Sebenza Hub WhatsApp Bot v2*
