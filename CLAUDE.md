...
- **Screenshots and image embeds:** Published image assets live in `01 How-To Documents/screenshots/` (inside the published surface), not `13-raw/screenshots/` — `13-raw/` is hidden on the public site and embeds from there won't render. Use filename-only embed syntax: `![[ch04-sidebar-hiring-path.png]]`, never `![[13-raw/screenshots/...]]` or a long path. Obsidian resolves filename-only embeds from anywhere in the vault, so embeds survive if the folder ever moves. Placeholder embeds with names like `r-ch04-billing-usage-page.png` are scattered through the manuals — produce a screenshot with that exact filename, drop it in `01 How-To Documents/screenshots/`, and it lights up automatically. The 11 originals also exist in `13-raw/screenshots/` (kept untouched per the immutability rule); any new screenshot intended for publication goes only in the public folder.
