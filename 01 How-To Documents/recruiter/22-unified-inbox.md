...
![[r-ch22-unified-inbox-multichannel.png]]
...
![[r-ch22-ai-email-composer-suggestions.png]]
...
![[r-ch22-outreach-draft-generator.png]]
...
**How to use it.** Pitch generation runs from inside the Unified Inbox composer — open a conversation with the candidate or client, click **Generate pitch**, pick the direction (to candidate / to client) and the linked role. The AI returns a structured pitch you can paste into any channel — email, WhatsApp, a client call notes prep doc.
...
![[r-ch22-whatsapp-messages-in-inbox.png]]

> **Linda runs WhatsApp generation now.** WhatsApp message generation in the inbox is handled by Linda — the platform's agent layer for adaptive multi-turn conversation. Practically, that means more context-aware drafts (Linda reads the candidate's recent thread and stage before writing) and a complete record of how the message was produced: every tool Linda invoked, with its inputs and outputs, lands in your Audit Trail (Chapter 28). You don't have to do anything different to use it; it's the engine behind the **Generate WhatsApp message** button.
